...
void setup() {
  ...
  #if NUM_SERIAL > 0
    MYSERIAL0.init(USART2,BOARD_USART2_TX_PIN,BOARD_USART2_RX_PIN);
    MYSERIAL0.begin(BAUDRATE);
    uint32_t serial_connect_timeout = millis() + 1000UL;
    while (!MYSERIAL0 && PENDING(millis(), serial_connect_timeout)) { /*nada*/ }
    #if HAS_MULTI_SERIAL
      MYSERIAL1.init(USART4,BOARD_USART4_TX_PIN,BOARD_USART4_RX_PIN);
  ...
  SERIAL_EOL();
    ...
    SERIAL_ECHOLNPAIR("Date:", __DATE__,"->Time:",__TIME__);
  #endif
  ...
  // (because EEPROM code calls the UI).
    ...
    DWIN_Frame_SetDir(0); // Orientation 90Â°
