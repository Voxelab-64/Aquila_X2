...
inline void Draw_Back_Label(void) {
  if (HMI_flag.language_flag)
  {
      DWIN_Frame_AreaCopy(1, 0, 100, 28,120,40,107);  //??
...
inline void Draw_Control_Menu() {
  ...
  if (HMI_flag.language_flag) {
     DWIN_Frame_AreaCopy(1,58,0,86,20,121,5); //??
  }
  else
  {
...
inline void Draw_Prepare_Menu() {
  ...
  if (HMI_flag.language_flag) {
      DWIN_Frame_AreaCopy(1,87,0,115,20,112,5); //??             
    if (CVISI(1)) 
    {
    ...
    if (CVISI(4)) 
    {
      DWIN_Frame_AreaCopy(1,57,200,85,220,LBLX, CLINE(4)); // Read Config
      DWIN_Frame_AreaCopy(1,113,200,140,220,LBLX+28, CLINE(4));
    }
    if(CVISI(5)) Prepare_Item_Lang(5);//DWIN_Frame_AreaCopy(1,62,122,120,138,LBLX, CLINE(4));   //
    if(CVISI(6))
    {
      DWIN_Frame_AreaCopy(1,0,220,83,240,LBLX,CLINE(6));
    } 
  }
  else {
   ...
   if (CVISI(4))  DWIN_Frame_AreaCopy(1,135,342,262,362,LBLX, CLINE(4));
   if (CVISI(5))   Prepare_Item_Lang(5);// Read Config
   if (CVISI(6))  DWIN_Frame_AreaCopy(1,69,362,200,382,LBLX, CLINE(6));//DWIN_Frame_AreaCopy(1, 275, 166, 330, 180, LBLX, CLINE(4)); //zoffset
  }
...
inline void Draw_Info_Menu() {
  Clear_Main_Window();
  Draw_Steady_State();
  DWIN_Draw_String(false, false, font8x16, Black, White, (DWIN_WIDTH - 36 * MENU_CHR_W) , 144, (char*)"Voxelab Aquila X2");
  DWIN_Draw_String(false, false, font8x16, Black, White, (DWIN_WIDTH - 36 * MENU_CHR_W) , 188, (char*)"220*220*250 mm");
...
inline void Draw_Move_Menu() {
 ...
 // 
  if (HMI_flag.language_flag) {
    DWIN_Frame_AreaCopy(1,58,20,100,40,112,5);//???
...
void Draw_NO_Bltouch_Menu(void){
      ...
      if(HMI_flag.language_flag)
      {
          ...
          DWIN_ICON_Show(ICON,25,160,225);                 //????
          DWIN_Frame_AreaCopy(1, 124, 50, 150,64, 170,225);  //??
      }
      else
      {
  ...
  if (encoder_diffState == ENCODER_DIFF_CW) {
    ...
    switch (select_control.now) {
      ...
      case 7: 
        ...
        Draw_matic_Menu();
        if(HMI_flag.language_flag)
        DWIN_ICON_Show(ICON2,39,20,160);
        else
        DWIN_ICON_Show(ICON2,40,20,160);
        break;
      case 8: 
          checkkey=Desizing;
          queue.inject_P(PSTR("M140 S0"));
...
void DWIN_CompletedMaticHate(void)
{
      if(checkkey==Feeding) 
      {
        ...
        {
//				 DWIN_Draw_Rectangle(1, Background_black,  0,  160, 272,  380);
          DWIN_ICON_Show(ICON2,42,20,160);
        }   
      ...
      if(checkkey==Desizing) 
      {
        if(HMI_flag.language_flag)
        {
         ...
         DWIN_ICON_Show(ICON2,45,56,260);
        }
        else
        {
           DWIN_ICON_Show(ICON2,46,43,260);
        }
        gcode.process_subcommands_now_P(PSTR("G92 Z0 E0"));
       gcode.process_subcommands_now_P(PSTR("G1 F800 Z10"));
